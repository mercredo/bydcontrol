...
//                console.log(daw.guiElement);
                $.each(daw.guiElement, function(key, element){
                    ...
                    element.options.change = function(v){
                        ...
                        cObj.bydSocket.sendOSC(key, v, function(oscObj, oscVal){
                            $('#byd-osc-out').val("/" + oscObj + " " + oscVal);
                        });
                    };
                    // refresh listener
...
//       };
       ...
       $.fn.addBydElementTo = function(interface){
//           bydObj.$.find('.cleaner').remove();
        ...
        this.addButton = function(innerTarget, options){
            ...
            $(btnHtml).dawButton(options).addBydElementTo(target);
...
//            $(knobHtml).dawKnob().appendTo(target);            
            $(knobHtml).dawKnob(options).addBydElementTo(target);
            // append attributes
...
//            $(sliderHtml).dawSlider().appendTo(target);            
            $(sliderHtml).dawHorizontalSlider(options).addBydElementTo(target);
            // append attributes
...
//            this.$.find('.cleaner').remove();
            ...
            var sliderHtml = '<input class=\'\' data-type=\'slider\' data-parent=\'' + this.getMembership(target) + '\' />';
//            $(sliderHtml).dawSlider().appendTo(target);            
            $(sliderHtml).dawVerticalSlider(options).addBydElementTo(target);
            // append attributes
            daw.guiElement["slider" + (sumSlider++)].parentGroup = target;
...
//            $(xyHtml).dawXY().appendTo(target);            
            $(xyHtml).dawXY(options).addBydElementTo(target);
            // append attributes
...
//            $(vgHtml).appendTo(this.elementWrapper().appendTo(target));            
            $(vgHtml).css(properties.css.default).append(this.cleaner()).insertBefore($(target).children().last());   
//            this.$.append(this.cleaner());
...
//            $(hgHtml).appendTo(this.elementWrapper().appendTo(target));            
            $(hgHtml).css(properties.css.default).append(this.cleaner()).insertBefore($(target).children().last());
//            this.$.append(this.cleaner());
